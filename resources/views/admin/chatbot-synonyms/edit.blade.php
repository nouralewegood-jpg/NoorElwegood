    ...
    <div class="breadcrumb-header justify-content-between">
        <div class="my-auto">
            <div class="d-flex">
                ...
                <span class="text-muted mt-1 tx-13 mr-2 mb-0">/ المترادفات / تعديل</span>
            </div>
        </div>
    </div>
    <!-- breadcrumb -->
        ...
        <div class="card">
            <div class="card-header">
                <div class="d-flex justify-content-between align-items-center">
                    <h3 class="card-title">تعديل المرادفات: {{ $chatbotSynonym->main_word }}</h3>
            ...
            <div class="card-body">
                ...
                <form action="{{ route('admin.chatbot-synonyms.update', $chatbotSynonym->id) }}" method="POST">
                    @csrf
                    @method('PUT')
                    <div class="form-group">
                        <label for="main_word">الكلمة الرئيسية <span class="text-danger">*</span></label>
                        <input type="text" name="main_word" id="main_word"
                            class="form-control @error('main_word') is-invalid @enderror"
                            value="{{ old('main_word', $chatbotSynonym->main_word) }}" required>
                    </div>
                    ...
                    <div class="form-group">
                        ...
                        <textarea name="synonyms" id="synonyms" rows="4" class="form-control @error('synonyms') is-invalid @enderror"
                            required>{{ old('synonyms', is_array($chatbotSynonym->synonyms) ? implode(', ', $chatbotSynonym->synonyms) : $chatbotSynonym->synonyms) }}</textarea>
                    ...
                    <div class="form-group">
                        <div class="custom-control custom-switch">
                            <input type="checkbox" class="custom-control-input" id="active" name="active" value="1"
                                {{ old('active', $chatbotSynonym->active) ? 'checked' : '' }}>
